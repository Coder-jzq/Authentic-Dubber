...
def evaluate_Denoise2_ID20Emo_Setting1(model, fusion_model, step, configs, val_log_path, logger=None, vocoder=None):
    ...
    emotions_batch = []
    cofs_batch = []
    val_samples_path = os.path.join(train_config["path"]["file"], train_config["path"]["result_path"],str(step))
    ...
    for batchs in loader:
        for batch in batchs:
            with torch.no_grad():
                ...
                Ture_postnet_mel_predictions, mel_lens_pred = model(feature, text_encoder, src_masks, ref_mels, ref_mel_lens, face_lens, MaxfaceL, lip_embedding, spk_embedding, mel_lens=mel_lens, max_mel_len=max_mel_len, mel_target=mel_target, infer_flag=True, scene_emo_embedding = scene_emo_embedding, face_emo_embedding = face_emo_embedding, text_emo_embedding = text_emo_embedding, scene_refs = scene_refs, face_refs = face_refs, text_refs = text_refs, scene_ref_audios = scene_ref_audios, face_ref_audios = face_ref_audios, text_ref_audios = text_ref_audios)
    ...
    print("============MCD=================")
    audio_path = "./wav_22050_chenqi_clean_Denoise_version2_all"  # change you path; We use the processed GT audio (e.g., Abs) as the target, which is used to convert mel-spectrograms.
...
def evaluate_GRID(model, fusion_model, step, configs, val_log_path, logger=None, vocoder=None):
    ...
    save_wav(sampling_rate, val_samples_path, wav_predictions_batch, tags_batch)
    print("============MCD=================")
    audio_path = "./Grid_Wav_22050_Abs"  # change you path; We use the processed GT audio (e.g., Abs) as the target, which is used to convert mel-spectrograms.
