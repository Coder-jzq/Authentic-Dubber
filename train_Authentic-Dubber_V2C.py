...
def main(args, configs):
    ...
    print("Number of the Authentic-Dubber Parameters:", num_param)
    ...
    # Load vocoder
    vocoder = bigvgan.BigVGAN.from_pretrained('./bigvgan_v2_22khz_80band_fmax8k_256x', use_cuda_kernel=False)
    ...
    while True:
        ...
        for batchs in loader:
            for batch in batchs:
                id_basename, text, src_len, max_src_len, speakers, ref_mels, ref_mel_lens, mel_target, mel_lens, max_mel_len, pitches, energies, durations, ref_linguistics, face_lens, MaxfaceL, lip_embedding, spk_embedding, face_embedding, emos_embedding, emotion_id, scene_emo_embedding, face_emo_embedding, text_emo_embedding, scene_refs, face_refs, text_refs, scene_ref_audios, face_ref_audios, text_ref_audios = model.parse_batch(batch)
                feature, src_masks, speaker_predicts, emotion_id_embedding, text_encoder = fusion_model(text, face_embedding, src_len, max_src_len, ref_mels, ref_mel_lens, face_lens, MaxfaceL, scene_emo_embedding, face_emo_embedding, text_emo_embedding, scene_refs, face_refs, text_refs, scene_ref_audios, face_ref_audios, text_ref_audios)
                mel_predictions, postnet_mel_predictions, log_duration_predictions, _, src_masks, mel_masks, _, ref_mel_masks, AV_attn, lip_masks, noise_loss, emotion_id_embedding = model(feature, text_encoder, src_masks, ref_mels, ref_mel_lens, face_lens, MaxfaceL, lip_embedding, spk_embedding, mel_lens=mel_lens, max_mel_len=max_mel_len, d_targets=durations, p_targets=pitches, e_targets=energies, mel_target = mel_target, infer_flag = False, scene_emo_embedding = scene_emo_embedding, face_emo_embedding = face_emo_embedding, text_emo_embedding = text_emo_embedding, scene_refs = scene_refs, face_refs = face_refs, text_refs = text_refs, scene_ref_audios = scene_ref_audios, face_ref_audios = face_ref_audios, text_ref_audios = text_ref_audios)

                total_loss, item_losses = Loss(text, speakers, mel_target, pitches, energies, mel_lens, durations, ref_linguistics, mel_predictions, postnet_mel_predictions, log_duration_predictions, src_masks, mel_masks, ref_mel_masks, speaker_predicts, spk_embedding, AV_attn, src_len, face_lens, lip_masks, emotion_id_embedding, emotion_id, noise_loss)
...
if __name__ == "__main__":
    ...
    # load Authentic-Dubbeer model_config and preprocess_config. (see "./ModelConfig_V2C/model_config/MovieAnimation/config_all.txt")
    preprocess_config = ast.literal_eval(preprocess_config)
    model_config = ast.literal_eval(model_config)

    # load Authentic-Dubbeer train_config.  (see "./config/MovieAnimation/train.yaml")
