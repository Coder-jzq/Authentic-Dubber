...
class GraphAttentionLayer(nn.Module):
    ...
    def __init__(self, in_features, out_features, dropout, alpha, concat=True, relation = False, num_relation=-1,relation_coding='hard',relation_dim=50):
        ...
        if self.relation:
            if relation_coding=='hard':
                emb_matrix = torch.eye(num_relation) 
                self.relation_embedding = torch.nn.Embedding.from_pretrained(emb_matrix, freeze = True)  
    ...
    def forward(self, h, adj):
        ...
        if self.relation:
            ...
            relation_one_hot = self.relation_embedding(adj)  
