...
class MULTModel(nn.Module):
    ...
    def forward(self, x_l, x_a, x_v, src_lens, max_src_len):
        # feature = fusion_model(text, emos_embedding, spk_embedding, src_len, max_src_len)
        """
        ...
        if self.partial_mode == 3:
            ...
            concat = last_h_l+last_h_a+last_h_v
        #print(concat.shape)
        concat = concat.unsqueeze(1).expand(-1, max_src_len, -1)
        #print(concat.shape)
        
        return (concat + text_encoder)/2, src_masks
