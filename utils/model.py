...
def get_model_ID22_E1_Duration_pretrain(args, configs, device, train=False):
    (preprocess_config, model_config, train_config) = configs
    """
    ...
    """
    model = Style_dubber_model_15_SPost_Duration(preprocess_config, model_config, train_config).to(device)
    ...
    if train:
        ...
        # Load the pre-trained checkpoint for training
        ckpt = torch.load("/sdc/data/zhaoyuan/Rag_Dubber/Ragdubber_top3_stage_1/ckpt/MovieAnimation/50000.pth.tar")
        model.load_state_dict(ckpt["model"], strict=False)
        fusion_model.load_state_dict(ckpt["fusion_model"], strict=False)

        # Freeze all layers in model and fusion_model
        for param in model.parameters():
            param.requires_grad = False
        for param in fusion_model.parameters():
            param.requires_grad = False
        
        # Unfreeze the matchatts_decoder module only
        for param in model.matchatts_decoder.parameters():
            param.requires_grad = True

        # Create the optimizer and set up the training configuration
        scheduled_optim = ScheduledOptim(
            model, fusion_model, train_config, model_config, args.restore_step
        )
        
        # Set the model and fusion model to training mode
        model.train()
        fusion_model.train()
...
def get_model_ID22_E1_Duration(args, configs, device, train=False):
    ...
    """
        ...
        model = Style_dubber_model_15_SPost_Duration ===========> Paper Figure 2, b) Phoneme-guided Lip Aligner (PLA) (Sec. 3.3), and c) Utterance-level Style Learning (USL) (Sec. 3.4)
    """
    model = Style_dubber_model_15_SPost_Duration(preprocess_config, model_config, train_config, model_config["Multimodal-Transformer"]).to(device)
    fusion_model = models_16_5_Face2_same.MULTModel(model_config["Multimodal-Transformer"], model_config, preprocess_config).to(device)
