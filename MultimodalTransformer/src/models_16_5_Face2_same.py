...
class MULTModel(nn.Module):
    def __init__(self, hyp_params, model_config, preprocess_config):
        ...
        self.linear_for_ref_mel = nn.Linear(128, 256)
    ...
    def forward(self, x_l, x_a, src_lens, max_src_len, ref_mels, ref_mel_lens, face_lens, MaxfaceL, scene_emo_embedding, face_emo_embedding, text_emo_embedding, scene_refs, face_refs, text_refs, scene_ref_audios, face_ref_audios, text_ref_audios):
        """
        ...
        if self.lonly:
            h_l_with_vs = self.trans_l_with_v(proj_x_l, proj_x_v, proj_x_v).transpose(0,1)

            ref_mel_seq = self.linear_for_ref_mel(h_l_with_vs)
           
        
        output = (ref_mel_seq + text_encoder) / 2

        return output, src_masks, spkemb, None, text_encoder
